...
Mỗi inference request được log theo thời gian thực: latency, token count, cost, error code. Ở quy mô production (hàng triệu call/ngày), mỗi micro-batch ghi ra một Parquet file nhỏ vào Bronze layer. Sau vài ngày, Delta table có thể chứa hàng chục nghìn file — mỗi file chỉ vài KB. Hệ quả trực tiếp: query p95 latency theo model/ngày phải scan toàn bộ file list từ `_delta_log`, overhead metadata đọc còn lớn hơn overhead đọc data thực.

NB2 minh họa rõ điều này: 200 appends nhỏ → query median tăng gấp ~3× so với sau khi `OPTIMIZE + Z-ORDER`. Ở production với 200.000 appends, tác động sẽ nghiêm trọng hơn nhiều.
